...
class Form
{
	 ...
	 */
	public function all() {
		return ee('Model')->get('formgrab:Form')->all();
	}
	...
	public function countAll() {
		return ee('Model')->get('formgrab:Form')->count();
	}
	...
	public function findOrCreate() {
		...
		$form = ee('Model')->get('formgrab:Form')->filter('name', ee()->input->post('formgrab_name'))->first();
		...
		if( is_null( $form ) ) {
			...
			$data = array(
				...
				'name' => ee()->input->post('formgrab_name'),
				'view_order' => $this->getNextViewOrder(),
				'created_at' => ee()->localize->now,
				'updated_at' => ee()->localize->now,
			);

			if( ee()->input->post('formgrab_title') ) {
				$data['title'] = ee()->input->post('formgrab_title');
			} else {
				$data['title'] = ee()->input->post('formgrab_name');
			}
	 ...
	 */
	public function saveSubmission( $form ) {
		...
		if( ee()->input->post('formgrab_required') ) {
			$required = explode('|', trim(ee()->input->post('formgrab_required')));
			$rules = array();
		...
		if (ee()->extensions->active_hook('formgrab_validate') === TRUE) {
			$post = ee()->extensions->call('formgrab_validate', $post);
		}
		...
		// } else {
			...
			if (ee()->extensions->active_hook('formgrab_save_submission') === TRUE) {
				$sub = ee()->extensions->call('formgrab_save_submission', $sub, $post);
			...
			if( $form->email_notification ) {
				...
				if( ee('Validation')->check('email', $to) == false ) {
